...
> 状态：实现与本地聚焦验证进行中；尚未提交、推送或创建 Draft PR。最后核验：
...
当前尚无 W19 commit、远端 branch、Draft PR 或 exact-head CI 证据。完成定义仍包括：全量本地质量门通过、只暂存 W19
文件、聚焦提交、推送、创建 Draft PR，并核验该 exact head 的必需检查。用户现有 `AGENTS.md` 无关改动必须保持未暂存，
不得进入 W19 PR。
