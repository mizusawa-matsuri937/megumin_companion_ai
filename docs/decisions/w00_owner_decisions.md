...
| OD-W00-19 | W30 既有本地 GPT-SoVITS Gateway 兼容 | 2026-07-31 明确授权 | 仅在独立 W30 工作树移植固定 loopback、专用令牌、无缓存/无代理的最小 client；不修改 W29，不自动回退 Mock，也不扩大 DeepSeek 出站字段 |
...
## W30 本地 Gateway 兼容授权（2026-07-31）

项目所有者确认：为恢复已存在的本机 `gpt-sovits-gateway` 配置，W30 可在自己的工作树中实现
最小兼容 client。其固定边界是数值 loopback `127.0.0.1:9880`、私有 bearer-token 协议、五个有限
voice slot、无音频缓存、无代理、无自定义 CA，且 token 必须保存在独立的 current-user DPAPI purpose 槽。

这项授权不批准把 Gateway 视为直连 GPT-SoVITS 的别名、启动或管理任何 Gateway 进程、把令牌放进 YAML/
事件/日志、用 Mock 掩盖错误，或改变 DeepSeek 的文本/视觉/记忆数据出口。W29 原工作树及其验收状态保持不变。
