...
> TTS 网关、真实中文播放和 VTS 联动已在本地工作树完成自动/实机验证，发布 CI 与主观试听待完成。受管
> 中文 STT runtime 已由 [`224e06f`](https://github.com/mizusawa-matsuri937/megumin_companion_ai/commit/224e06f9cbb1d2ab0cc2260fb244b1f74cd7dfbc)
> 加入并完成该 code head 的 CI；其他行仍不代表代码已经实现。
...
| API key/VTS token/TTS gateway token | Secret store | provider adapter 只在使用时读取；gateway token 只到 loopback Authorization | DPAPI current-user 密文文件 | 仅发给对应 endpoint/协议 | replace/revoke/reset；普通卸载默认删除 | 不记录值、密文、header 或路径 |
...
7. W29 gateway 不由主程序自动启动；关闭桌面启动器必须关闭 Job Object 并清理整个推理子进程树。网关
