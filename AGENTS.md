...
- 本项目当前由所有者明确限定为**单机、单 Windows 用户、个人私用**。除非所有者明确扩大产品范围，多用户、跨用户 SID/DACL 有效访问、RDP/快速切用户和跨 session 行为均为范围外；记录时必须写明“范围外”，不得写成已通过，也不得重新列为人工 Gate。
- 人工清单只能保留 AI 无法可靠复现的实际桌面 shell 视觉/交互、真实硬件或其他明确批准的体验项。配置读取、注册表协调、生命周期、取消/关闭竞争、可控时钟和可合成故障必须先由 AI 自动化并附命令与结果；不得把它们转交人工手工配置或重复操作。
